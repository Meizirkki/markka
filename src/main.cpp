...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Markka: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Markka Signed Message:\n";
...
{
    // Markka: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Markka: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 7 * 24 * 60 * 60; // Markka: 7 days
static const int64 nTargetSpacing = 5 * 60; // Markka: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Markka: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Markka: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Markka: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2b952c073a932babe302bc4caeac289ced540c6bf4a98a468924ebb05ea001bc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Kadunvaltaajat pysäytti liikenteen stadissa";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 414000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f93d54b7873cad866b9d9670717ce5c2d11715e27e35ae55d0b4ee2eb106c9e49d5e8e874b53604f9c8ae6620b65f685ef9ca90af4686b7f5e0257ae26fd4ce0") << OP_CHECKSIG;
        CBlock block;
        ...
        assert(block.hashMerkleRoot == uint256("0x31dfecdbb77c4f593ebfc41c713e3e0ab0728739f4c154442157b9d9ca34287b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Markka: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MarkkaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MarkkaMiner : ProcessBlock, block not accepted");
    }
